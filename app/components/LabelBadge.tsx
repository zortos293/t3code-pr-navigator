export default function LabelBadge({ label }: { label: string }) {
  const getBadgeStyle = (l: string) => {
    ...
    if (normalized.includes('bug')) return 'bg-red-100 text-red-800 border-red-300 dark:bg-red-900/40 dark:text-red-300 dark:border-red-700';
    if (normalized.includes('enhancement') || normalized.includes('enchancement') || normalized.includes('feature')) return 'bg-cyan-100 text-cyan-800 border-cyan-300 dark:bg-cyan-900/40 dark:text-cyan-300 dark:border-cyan-700';
    if (normalized.includes('help')) return 'bg-yellow-100 text-yellow-800 border-yellow-300 dark:bg-yellow-900/40 dark:text-yellow-300 dark:border-yellow-700';

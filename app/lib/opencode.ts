...
function getOpenCodeTimeoutMs(): number {
  loadLocalEnv();
  const rawValue = process.env.OPENCODE_GO_TIMEOUT_MS?.trim();
  if (!rawValue) {
...
function isOpenCodeTimeoutError(error: unknown): error is Error {
  return error instanceof Error && error.message.includes('timed out');
}

function splitItemsInHalf<T>(items: T[]): [T[], T[]] {
...
async function callOpenCodeGoAPI(prompt: string): Promise<string> {
  ...
  try {
    ...
    if (error instanceof Error && (error.name === 'AbortError' || error.name === 'TimeoutError')) {
      throw new Error(`OpenCode Go request timed out after ${timeoutMs / 1000} seconds`);
    }
...
export function splitRelationshipBatch(batch: RelationshipBatch): RelationshipBatch[] {
  if (batch.issues.length <= 1 && batch.pullRequests.length <= 1) {
    return [batch];
  }

  if (batch.pullRequests.length >= batch.issues.length && batch.pullRequests.length > 1) {
    const [firstHalf, secondHalf] = splitItemsInHalf(batch.pullRequests);
    return [
      { issues: batch.issues, pullRequests: firstHalf },
      { issues: batch.issues, pullRequests: secondHalf },
    ].filter((candidate) => candidate.pullRequests.length > 0);
  }

  const [firstHalf, secondHalf] = splitItemsInHalf(batch.issues);
  return [
    { issues: firstHalf, pullRequests: batch.pullRequests },
    { issues: secondHalf, pullRequests: batch.pullRequests },
  ].filter((candidate) => candidate.issues.length > 0);
}

export function splitDuplicateBatch(batch: DuplicateBatch): DuplicateBatch[] {
  if (batch.primaryIssues.length <= 1 && batch.comparisonIssues.length <= 1) {
    return [batch];
  }

  if (batch.primaryIssues.length >= batch.comparisonIssues.length && batch.primaryIssues.length > 1) {
    const [firstHalf, secondHalf] = splitItemsInHalf(batch.primaryIssues);
    return [
      { primaryIssues: firstHalf, comparisonIssues: batch.comparisonIssues },
      { primaryIssues: secondHalf, comparisonIssues: batch.comparisonIssues },
    ].filter((candidate) => candidate.primaryIssues.length > 0);
  }

  const [firstHalf, secondHalf] = splitItemsInHalf(batch.comparisonIssues);
  return [
    { primaryIssues: batch.primaryIssues, comparisonIssues: firstHalf },
    { primaryIssues: batch.primaryIssues, comparisonIssues: secondHalf },
  ].filter((candidate) => candidate.comparisonIssues.length > 0);
}

async function analyzeRelationshipBatchOnce(batch: RelationshipBatch): Promise<RelationshipMatch[]> {
...
async function analyzeDuplicateBatchOnce(batch: DuplicateBatch): Promise<DuplicateMatch[]> {
...
async function analyzeRelationshipBatch(batch: RelationshipBatch): Promise<RelationshipMatch[]> {
  try {
    return await analyzeRelationshipBatchOnce(batch);
  } catch (error) {
    if (!isOpenCodeTimeoutError(error)) {
      throw error;
    }

    const smallerBatches = splitRelationshipBatch(batch);
    if (smallerBatches.length === 1) {
      throw new Error(
        `${error.message}. Timed out even on the smallest relationship batch. Increase OPENCODE_GO_TIMEOUT_MS if the provider is slow.`
      );
    }

    const results = await Promise.all(smallerBatches.map((smallerBatch) => analyzeRelationshipBatch(smallerBatch)));
    return results.flat();
  }
}
...
async function analyzeDuplicateBatch(batch: DuplicateBatch): Promise<DuplicateMatch[]> {
  try {
    return await analyzeDuplicateBatchOnce(batch);
  } catch (error) {
    if (!isOpenCodeTimeoutError(error)) {
      throw error;
    }

    const smallerBatches = splitDuplicateBatch(batch);
    if (smallerBatches.length === 1) {
      throw new Error(
        `${error.message}. Timed out even on the smallest duplicate batch. Increase OPENCODE_GO_TIMEOUT_MS if the provider is slow.`
      );
    }

    const results = await Promise.all(smallerBatches.map((smallerBatch) => analyzeDuplicateBatch(smallerBatch)));
    return results.flat();
  }
}
